...
\newglossaryentry{Eindeutigkeitssatz fur stetige Funktionen}{
	name = {Eindeutigkeitssatz fur stetige Funktionen},
	description = { Es seien $(X,d),(Y,h)$ metrische Räume und $f,g:X \to Y$ stetige Funktionen 
		für die $f|_D = g|_D$ gelte, wobei $D\subseteq X$ eine dichte Teilmenge ist.\\
		Dann gilt auch schon $f=g$ auf ganz $X$. Insbesondere gilt: Ist $f$ auf $D$ 
		stetig, lässt es sich auf ganz $X$ eindeutig stetig fortsetzen. },
	sort = stetig
}
...
\newglossaryentry{Diagonalverfahren}{
  ...
  description = {Diagonalverfahren}	
}

\newglossaryentry{glm Konvergenz und Ableitung}{
	name = {gleichmaessige Konvergenz und Ableitung},
	description = {%
		Es sei $(f_n)$ eine Folge von stetig differenzierbaren Funktionen auf 
		einem kompakten Intervall. Es gelte $f_n \to f$ punktweise für ein stetig 
		differenzierbares $f$ und $\norm{f'_n - g}_\infty \to 0$ 
		für ein stetiges $g$.\\
		Dann ist $f' = g$.  }
}
