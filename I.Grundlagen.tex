...
\begin{motivation}
\footnotesize
Was macht man in der Funktionalanalysis? man definiert sich einen vernünfitgen Abstandsbegriff: z.B. den Begriff der \glslink{metrischer_Raum}{Metrik} auf (zunächst einmal) einer beliebigen Mengen $E$. Damit hat man einen \glslink{metrischer_Raum}{metrischen Raum} geschaffen.
\end{motivation}

\paragraph{Arsenal an Begriffen}
Der nötige Apparat für die Entwicklung der Funktionanalysis ist nicht groß,
er besteht im wesentlichen aus den Begriffen \gls{metrischer_Raum},
\glslink{topologisch}{Topologie}, \gls{Konvergenz}, \glslink{CF}{Cauchy-Folge} und \glslink{stetig}{Stetigkeit}.\par
Einige weitere Begriffe sind in \ref{bem:1.28} und \ref{bem:1.30} festgehalten. \par
Es sei stets für das ganze Skript $\K \in \set{\R, \C}$.
%Definition der Seminorm
\begin{definition}[Halbnorm, Seminorm]
	\label{def:1.1}
	Sei $X$ ein $\K$-Vektorraum. 
	Für $x,y\in X$, $\lambda \in \K$ ist eine \textit{Halbnorm} (oder Seminorm) eine Abbildung
...
\begin{bem}[Kern und Quotientenraum von Halbnormen]
	\label{bem:1.2}
	\begin{enumerate}[(a)]
	...
	\end{enumerate}
	Siehe \hyperref[B4.1]{Aufgabe 1 auf Blatt 4}.
\end{bem}
...
\begin{bsp}[wichtige Vektorräume]
	...
	Sei $(\Omega,\hA,\mu)$ ein Maßraum
		\begin{enumerate}[(a)]
			\item Für $p\in[1,\infty)$. Definiere seminormierten Raum durch 
				$$\hL^p(\Omega,\mu) = \set{f:\Omega \rightarrow \C \text{ messbar}:
					\int_\Omega |f|^p d\mu < \infty}\; 
				\text{ mit } \seminorm{f}_p := (\int_\Omega |f|^p d\mu )^{\frac{1}{p}}.$$
				$L^p(\Omega,\mu)$ ist ein vollständiger normierter Raum ($\nearrow$ Ana III).

			\item Für $p = \infty$ gibt es mit $\seminorm{f}_\infty :=
				\underset{x\in\Omega}{\ess \sup} |f(x)|$ ebenfalls 
				einen seminormiert Raum
				$$\hL^\infty(\Omega,\mu) := \set{f:\Omega \rightarrow \C 
				\text{ messbar und \glslink{ess beschrk}{essentiell beschränkt} }}.$$ 
						$L^\infty(\Omega,\mu)$ ist ein vollständiger normierter Raum.

			\item Sei $p\in [1,\infty]$ und $|\cdot|$ das Zählmaß auf $\N$ und der Maßraum sei gegeben durch 
						$(\N, P(\N), |\cdot|)$.\\
...
%Trivia Fakten
	...
	\label{bem:1.4}
		...
		\begin{enumerate}[(a)]
			\item $L^p(\Omega,\mu)$ ist ein Banachraum, 
				$L^2(\Omega,\mu)$ ist ein Hilbertraum mit 
				$\disp (f,g)_2 := \int_\Omega f \overline{g} d\mu$
...
\subsection{Definitionen und grundlegende Eigenschaften}
%Definition linearer Operator
	...
	\label{def:1.5}
		...
		Wenn $X,Y$ normierte $\K-$Vektorräume sind, heißt ein linearer Operator
		$T$ \textit{beschränkt}, wenn \linebreak $T(U_1(0)) \subseteq Y$ beschränkt ist.
		In Quantoren lautet diese Aussage
	...
	\begin{thm}[Charakterisierung der Stetigkeit]
	\label{thm:1.7}
...
An dieser Stelle werden einige Definition und Bemerkung festgehalten, die in späteren Kapitel beleuchtet und untersucht werden.
	...
	\begin{bem}[wichtige Fakten zu Linearen Abbildung]
	\label{bem:1.9}
		\begin{enumerate}[a)] \addtocounter{enumi}{3}
			...
			\item Es gibt beschränkte lineare Operatoren, so dass $Im\: T$ 
				nicht abgeschlossen ist. $\nearrow$ \hyperref[B2.1]{B2,A1}
			\item Für alle $T\in \B(X,Y)$ ist $ker T$ abgeschlossen, 
				da $T$ stetig ist und der $ker\:T = \inv{T}(\{0\})$,
				also Urbild einer abgeschlossen Menge unter einer stetigen Funktionen. 
		\end{enumerate}
	\end{bem}						
...
%%%%%%%%%%%	Satz 1.10
	...
	\begin{bem}[Äquivalenz der Operatornorm]
	\label{bem:1.11}
		Es gilt 
		$\disp \norm{T} = \supT[A]\norm{Tx}_Y = \supT[R] \norm{Tx}_Y
		= \sup_{\overset{x \in X}{x \not = 0}} \frac{\norm{Tx}_Y}{\norm{x}_X}$
		($\nearrow$ \hyperref[B1.2]{Ü1, A2}).
...
%%%%%%%%%%% Satz 1.12
	\begin{thm}[Y Banachraum, $\BS$ Banachraum]
	\label{thm:1.12}
...
%%%%%%% Korollar 1.13
	\begin{cor}[Dualraum ist ein Banachraum]
	\label{cor:1.13}
		$X$ normierter Raum $\df$ $X'$ Banachraum.
	\end{cor}

%%%%%%% Bemerkung 1.14
	\begin{bem}[wichtige (negative) Resultate]
	\label{bem:1.14}
		\begin{enumerate}[a)]
			...
			i.A. \underline{nicht} $\disp \limes T_n = T$ (d.h. $\limes \norm{T_n - T} = 0$).
				\begin{description} \item[Bsp:] 
					Sei $X = \ell^p$, $p\in [1,\infty)$, 
					$x = (x_k)_{k\in\N} = (x_1,\dots,x_n,\dots)\in\ell^p$ und 
					$$T_n:\ell^p \to \ell^p, \; T_n x = (x_1,\dots,x_n,0,0,\dots)$$ 
					Es gilt $T_n \in \B(x)$ $\forall n\in\N$ (nachrechnen!) und 
					$\forall \epsilon > 0 \; \exists N\in\N \; \forall n \geq N: $
					$$\norm{T_n(x_k) - x_n}_X = \left(\sum_{k=n+1}^\infty 
						|x_k|^p\right)^{\frac{1}{p}} < \varepsilon \;
						\df \norm{T_n - x}_X \to 0 \; (n\to \infty).
						$$
					Aber! Betrachte	$\norm{T_ne_{n+1} - e_{n+1}}_X = \norm{e_{n+1}}_X = 1$ 
				Also gilt $\norm{I - T_n} \geq 1$.
				\end{description}
		...
		\item $T\in \BS$ und $T$ bijektiv. Dann ist $\inv{T}$ i.A. \underline{nicht beschränkt}.
			\begin{description} 
				...
				\begin{itemize}
					\item $\inv{T}=S: Y\to X, Sy = y'$. (Zeige $ST = I_X$ und $TS = I_Y$)
					\item $\inv{T}\not\in\B(Y,X)$ (Sei $y_n(t) = t^n\in Y$,
					$(\inv{T}y_n)(t) = n\cdot t^{n-1}$
					$\df \norm{y_n}_Y =1 \; \forall n\in \N$, $\norm{\inv{T}y_n}_X =n 
					\; \forall n\in\N \df \inv{T}$ kann nicht beschränkt sein. 
				\end{itemize}
				Bem: $Y$ ist nicht vollständig. Wir werden sehen, dass dies wesentlich ist.
			\end{description}
		\end{enumerate}
	\end{bem}

%%%%%%% Satz 1.15
	\begin{thm}[Charakterisierung stetige Inverse und Injektivität]
	\label{thm:1.15}
		Sei $X,Y$ normierte $\K$-VR, $T\in \BS$. Dann sind äquivalent:
			\begin{enumerate}[(i)]
				\item $T$ ist injektiv und $\inv{T} \in\B(im(T), X)$. 
...
%%%%%%% Korollar 1.16
	\begin{cor}[Nichtexistenz der stetigen Inversen]
	\label{cor:1.16}
		Seien $X,Y$ normierte $\K$-VR, $T \in \BS$. Dann sind äquivalent:
			\begin{enumerate}[(i)]
...
%%%%%%% 1.18
	\begin{thm}[Charakterisierung der Äquivalenz von Normen]
	\label{thm:1.18}
...
%%%%%%% 1.19
	\begin{definition}[topologischer Isomorphismus]
	\label{def:1.19}
	...
	\begin{thm}[Charakterisierung topologisch isomorph]
	\label{thm:1.20}
		$X, Y$ topologisch isomorph $\aq$ $\exists m,M > 0: T\in \BS$ und injektiv
		$: m\norm{x}_X \leq \norm{Tx}_Y \leq M \norm{x}_X \; \forall x\in X$
	\end{thm}
	\begin{proof}
		'Klar' wegen \hyperref[thm:1.18]{Satz 1.18} und \hyperref[thm:1.15]{Satz 1.15}.
	\end{proof}
	
	\begin{bem}[Isometrie und lineare Bijektion]
	\label{bem:1.21}
		\begin{enumerate}
			\item Falls, $m=M=1$, dann nennen wir $T$ {\it Isometrie}. Also 
				$\norm{Tx} = \norm{x}$.
			\item Falls $\dim X = \dim Y = n\in \N$: $X,Y$ topologisch isomorph 
				und topologischer Isomorphismus = lineare Bijektion.
		\end{enumerate}
	\todog[Isometrie: Metrik bleibt erhalten, Isomorphismus: im Sinne der LA, 
		Normisomorph: beides]
	\end{bem}
...
%% Satz 1.22
	...
	\label{thm:1.22}
		$X,Y$ normierter Raum, $Y$ ein Banachraum, $Z\subseteq X$, $Z$ dichter UVR.
...
%% Satz 1.23
	\begin{thm}[Fortsetzung bleibt normerhaltend]
	\label{thm:1.23}
		Ist $T$ normerhaltend (in $\R^n$ die unitären Matrizen $\norm{Tx} = \norm{x}$),
		so ist $\tilde{T}$ ebenfalls normerhaltend.
	\end{thm}
	...
	\end{thm}
		\begin{proof}
			\begin{itemize}[]
				...
				\item $"' iii) \df iv)"':\;$ Sei $n\in\N \df \exists \ell\in\N, 
					k\in \{0,\dots,N-1\}$, 
					so dass $n = \ell \cdot N + k$ Dann folgt  
					$$\ell \leq \frac{n}{N} -1\df \norm{T^n} = \norm{(T^N)^\ell T^k} \leq \norm{T^N}^\ell \cdot \norm{T^k}. $$
					Sei $M := \max\{1,\norm{T},\norm{T^2},\dots,\norm{T^{N-1}}\}$.
					Dann gilt $\norm{T^n} \leq M\norm{T^N}^\ell$ und damit folgt
					\spcm[-0.75] $$ \sqrt[n]{\norm{T^n}} 
					= 	 \sqrt[n]{\norm{T^N}^\ell}\sqrt[n]{M}
					\leq \sqrt[n]{\norm{T^N}^{\frac{n}{N}-1}} \cdot \sqrt[n]{M}
					= 	\underbrace{\sqrt[N]{\norm{T^N}}}_{< 1} \cdot 
						\underbrace{\sqrt[n]{\frac{M}{\norm{T^N}}}}_{\overset{\to 1}{\text{für } n\to\infty}} \spcm[-1]$$
		...
		\end{proof}
%%%%%%%%%%%%%%%%%%  
	\begin{bem}[Konvergenz Neumannsche Reihe]
	\label{bem:1.26}
		\begin{enumerate}
...
\section{Metrische und topologische Räume, Satz von Baire}
	...
	\subsection{Grundbegriffe:metrischer Raum und Vervollständigung}	
...
%Lemma 1.29
	\begin{lemma}[Vierecksunsgleichung]
	\label{lem:1.29}
...
%Bem 1.30
	...
	\label{bem:1.30}
		$U_r(x)$ \gls{Kugel mit Radius r},	
		\glslink{Abschluss}{$\overline{M}$ Abschluss},	
		\glslink{Inneres}{$\mathring{M}$ Innere},	
		\glslink{Rand}{$\partial M$ Rand},
		\glslink{kompakt}{Kompakt},
		\glslink{offene Uberdeckung}{offene Überdeckung}.
	\end{bem}
...
\begin{definition}[Abstandserhaltung, Einbettung und Isometrie]
	...
	\begin{enumerate}[(a)]
		...
		\item \textit{Isometrie} falls abstandserhaltend  und surjektiv, also insgesamt
			bijektiv. Aus abstandserhaltend folgt die Injektivität. 
	\end{enumerate}
	Das rechtfertigt auch folgende Begriffsbildung:\\
	Eine abstandserhaltende  Abbildung heißt auch \textit{Einbettung}.
	Eine Einbettung heißt \textit{dicht}, falls $f(X)$ dicht in Y ist.\par
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Der Bairesche Kategoriesatz und grundlegendes zu Vollständigkeit und Kompaktheit}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{definition}[Durchmesser]
	...
	Wir definieren den {\it Durchmesser} von M durch
...
%Einschubskorollar ohne Nummerierung
	\begin{cor*}
		Der Raum der Polynome $\mathcal{P} $ kann mit keiner Norm vollständig sein.
	\end{cor*}
	\begin{proof}
		Es sei $b_n := x^n$. Die Familie $(b_n)_{n\in \N}$ bildet eine abzählbare Basis von $\mathcal{P}$. Den Rest erledigt uns die Hausaufgabe \nameref{B4.2}.
	\end{proof}

%Definition 1.36
\begin{definition}[dicht, mager, Kategorie]
	\label{def:1.36}
...
\begin{motivation}
\begin{mbem*}[Trivia am Rande] Direkt aus der Definition folgt, das jede 
	nirgends dichte Menge insbesondere von 1.Kategorie ist. Andersrum gilt dies nicht,
	was das Beispiel $\Q\subseteq \R$ zeigt. Ein Beispiel für eine nirgends dichte Menge 
	ist die Cantor-Menge oder $\N\subset\R$.\\
	 \textit{Anschaulich} bedeutet \textit{nirgends dicht}, wenn sie in keiner Teilmenge (mit nichtleeren Innerem) dicht liegt. \par\bigskip
\end{mbem*}
...
%Korollar 1.37
\begin{cor}[vollstängier Raum von 2. Kategorie]
	\label{cor:1.37}
...
%Korollar 1.38
\begin{cor}[magere Unteremengen in vollständigen Raum]
	\label{cor:1.38}
...
%Korollar 1.39
\begin{cor}[Darstellung vollständiger metrischer Räume]
	\label{cor:1.39}
	$(X,d)$ sei ein vollständiger metrischer Raum. 
	Für $n\in\N$ sei $B_n \subseteq X$ so dass $X\setminus B_n$ mager. 
	$B:=\cap_{n\in\N}  B_n$
	$$\df \overline{B} = X$$
\end{cor}
\begin{proof}
...
%Definition 1.40
\begin{definition}[kompakt, präkompakt]
	\label{def:1.40}
	Der metrische Raum $(X,d)$ heißt \dots
	\begin{enumerate}[(a)]
	\item \textit{kompakt}, wenn für alle offenen Überdeckungen $(U_i)_{i\in I}$ 
		von $X$ ein endliches $I'\subseteq I$ existiert, 
		so dass $X = \cup_{i\in I'} U_i$
	\item \textit{präkompakt}, wenn $\forall \varepsilon > 0$ eine endliche Menge 
		$M = \{x_1,\dots, x_n\}$ existiert, so dass $X = \cup^n_{i=1} U_\varepsilon (x_i)$. 
		$M$ heißt auch $\varepsilon$\textit{-Netz} von $X$.
		\footnote{Äquivalent dazu: Jede Folge besitzt eine Teilfolge, die eine Cauchyfolge ist.}
\end{enumerate}		
Besonders interessant sind diese Begriffe für Teilräume $F\subset X$.
\end{definition}

%Satz 1.41
\begin{thm}[Charakterisierung von Kompaktheit]
	\label{thm:1.41}
...
%Korollar 1.42
	\begin{cor}[Folgen- und Überdeckungskompakt]
	\label{cor:1.42}
...
%Definition 1.43
	...
	\label{def:1.43}
		...
		$M\subseteq X$ heißt \textit{relativ kompakt}, wenn $\overline{M}$ kompakt ist.
	\end{definition}

%Definition 1.44
	\begin{thm}[Charakterisierung relativ kompakt]
	\label{thm:1.44}
...
%Satz 1.45
	\begin{thm}[Aussagen zu komapkt, prä- und  relativkompakt]
	\label{thm:1.45}
	...
	\label{bem:1.46}
		...
		\begin{enumerate}[a)]
			...
			\item Wenn $\dim X < \infty$, dann
				\begin{enumerate}[i)]
					\item $X$ Banachraum
					\item $M\subseteq X$ ist kompakt $\aq$ $M$ ist beschränkt und abgeschlossen (Heine-Borel)
					\item $M\subseteq X$ relativ kompakt $\aq$ $M$ präkompakt  $\aq$ $M$ beschränkt
				\end{enumerate}
		\end{enumerate}
	\end{bem}
\begin{motivation}
  Folgendes Lemma dient - neben immament Interessantem - zur Vorbereitung auf eine 
	Charakterisierung des Satzes Bolazano-Weierstraß, der genau dann gilt, 
	wenn der Raum von endlicher Dimension ist.
\end{motivation}
	...
	\begin{proof}
		Sei $x_0 \in X\setminus E$. Definiere $\disp \delta := \inf_{y\in E} 
		\norm{x_0 - y}$, da E abgeschlossen ist, ist $\delta > 0$. 
		Sei $(y_n)$ Folge in $E$ mit $\norm{x_0 - y_n} \to \delta$. 
		Da $\eta \in (0,1)$ ist $\frac{\delta}{\eta} > \delta $; also $\exists z\in E$ 
		mit $\norm{x_0 - z} \leq \frac{\delta}{\eta}.$ 
		Definiere $\xt := \frac{x_0-z}{\norm{x_0 - z}}$, dann ist $\norm{\xt} = 1$.
		Für $y\in E$ gilt dann 
		%\spcm[-1.3]
		\begin{flalign*}
		\norm{y -\xt} & = \norm{y - \frac{x_0-z}{\norm{x_0 - z}}} 
		= \norm{y + \frac{z}{\norm{x_0 - z}} - \frac{x_0}{\norm{x_0 - z}}} \\
		&= \frac{\norm{\;\left(\norm{x_0 - z}y + z\right) - x_0}}{\norm{x_0 - z}}  
...
%		%\end{split} \end{equation*}
	...
	\begin{motivation}
		\footnotesize
		\enquote{%
			Wir können nun die Endlichkeit der Dimension eines normierten Raumes (eine 
			{\sl algebraische} Eigenschaft) durch eine {\sl metrische } Eigenschaften 
			(die Gülitgkeit des Bolzano-Weierstraßschen Satzes) charakterisieren.
			Dass so etwas überhaupt möglich ist, liegt letztlich daran, dass die 
			algebraische und metrische Struktur normierter Räume nicht unverbunden
			nebeneinander stehen, sondern via Stetigkeit ineinanderengreifen.
		}
		[Heuser (S.104)]
	\end{motivation}
...
%Korollar 1.48
	\begin{cor}[kompakt in endlichdim. Räumen, konvergente Teilfolgen]
	\label{cor:1.48}
		$(X,\norm{\cdot})$ normierter Raum.
			\begin{enumerate}[a)] 
...
%Bemerkerung 1.49
	\begin{bem}[Diverses zu Skalarprodukträumen]
	\label{bem:1.49}
		\begin{enumerate}[1)]
	...
	\end{bem}
%Satz 1.50
	\begin{thm}[Skalarprodukt induziert Norm]
	\label{thm:1.50}
	...
	\end{thm}
	\begin{proof}
		($\nearrow$ \hyperref[B6.1]{Übung, Blatt 6, Aufgabe 1})
		Skizze! a) $\K = \R$ $(x,y) := \frac{1}{4}(\norm{x + y}^2 - \norm{x-y}^2)$.\\ 
		b) $\K = \C$ $(x,y) := \frac{1}{4}(\| x+y\|^2 - \|x-y\|^2 + 
				i(\|x+iy\|^2 + \|x-iy\|^2)$ 
	\end{proof}

% Definition 1.51
	\begin{definition}[Alles Orthogonlität inkl. System und Basis]
	\label{def:1.51}
...
%Satz 1.53 (Besselsche Ungleichung I)
...
%Korollar 1.54
	\begin{cor}[Fourreihe und Besssel Ungl. II]
	\label{cor:1.54}
...
%Korollar 1.56
	\begin{cor}[Orthogonalunterräume und Approximation]
	\label{cor:1.56}
...
%Lemma 1.57
	\begin{lemma}[Orthogonalraum und Abschluss]
	\label{lem:1.57}
...
%Satz 1.58
 	\begin{thm}[Charakterisierung von ONBs]
	\label{thm:1.58}
...
%Korollar, Folgerungen ohne Nummerierung.
	\begin{cor*}[Bestapproximation trigono. Polynome]
Sei $f\in L^2([0,2\pi])$
		\begin{enumerate}
...
%Definition 1.60 (Halbordnung, Totalordnung)
	...
	\label{def:1.60}
		Sei $M$ eine Menge. Eine Relation $\leq\; \subseteq M\times M$, 
		heißt \textit{Halbordnung}, wenn folgende Eigenschaften erfüllt sind:
			\begin{enumerate}[(i)]
...
%Satz 1.62
	\begin{thm}[Existenz von ONB in Hilberträumen]
	\label{thm:1.62}
	...
	Siehe Übung. Blatt \hyperref[B5.3]{5}, \hyperref[B6.2]{6}.

%Satz 1.63
	\begin{thm}[Basen in Hilberträumen haben die gleich Mächtigkeit]
	\label{thm:1.63}
	...
	\begin{proof}
		...
		Ist $i\in I$ mit $i \not \in \bigcup_{j\in J} S_{y_j}$, 
		dann $x_i \perp y_j \forall j\in J \df x_i = 0$ $\lightning$! \par
