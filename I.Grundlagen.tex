...
%%%%%%% 1.19
	...
	\label{bem:1.21}
		\begin{enumerate}
			\item Falls, $m=M=1$, dann nennen wir $T$ {\it Isometrie}.
...
%Bem 1.30
	...
	\label{bem:1.30}
		$U_r(x)$ Kugel mit Radius $r$,	$\overline{M}$ Abschluss,	$\mathring{M}$ 
		Innere,	$\partial M$ Rand,	Kompakt,	offene Überdeckung. \todoo[Hinten noch definieren]
	\end{bem}
...
%Definition 1.44
	...
	\label{def:1.44}
		$(X,d)$ vollständiger metrischer Raum, $M \subseteq X$ relativ kompakt. \par
					\todo[inline]{Was genau wird hier definiert?}
...
%Satz 1.62
	...
	\end{proof}
	Übung. Blatt \hyperref[B5.3]{5}, \hyperref[B6.2]{6}.
	\begin{enumerate}
		\item Gram-Schmidt-Orthonormalisierung\\
			Sei $(X, (\cdot,\cdot))$ ein Skalarproduktraum, $(x_n)_{n \in \N}$ linear unabhängig. 
			Dann existiert ein ONS $(y_n)_{n \in \N}$ mit $span(y_1,\dots,y_k)=span(x_1,\dots,x_k)$.\\ 
			Mit $(y_n)$ wie vorhin, gilt $(y_n)$ ist ONB $\forall k \in \N 
			\Leftrightarrow span(x_n)_{n \in \N} = X$.
