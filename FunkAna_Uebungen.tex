...
Es sei $J$ eine beliebige nichtleere Menge und
	...
	\begin{enumerate}[(a)]
		\item 
		Die Eigenschaften des Skalarproduktes ergeben sich sofort. $X$ ist auch ein Vektorraum. Denn sei $N(f) := \{ x \in J : f(x) \not = 0\}$, dann ist für $f,g \in X$ $N(f+g) \subseteq N(f) \cup N(g)$	und $N(\alpha f) \subseteq N(f)$ für $\alpha \in \R$,	also $f+g\in X$ und $\alpha f \in X$. Es ist noch die Vollständigkeit zu zeigen. Sei dafü
